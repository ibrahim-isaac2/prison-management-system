...
export default function ReleasedPrisonerCard({ prisoner, onEdit, onDelete }: ReleasedPrisonerCardProps) {
  ...
  return (
    <Card className="mb-4 overflow-hidden border-2 border-border/50 hover:border-emerald-500/30 transition-all duration-300 hover:shadow-xl hover:-translate-y-1 bg-gradient-to-br from-card via-card to-emerald-50/20">
      <CardHeader className="bg-gradient-to-r from-emerald-500/10 via-green-600/5 to-transparent pb-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <CardTitle className="text-right text-xl font-bold bg-gradient-to-l from-emerald-600 to-green-800 bg-clip-text text-transparent">
              {prisoner.name}
            </CardTitle>
          </div>
          <div className="flex items-center gap-2">
            <Badge className="bg-gradient-to-r from-emerald-500 to-green-600 text-white shadow-md px-3 py-1 text-sm font-semibold">
              {prisoner.prison}
            </Badge>
      ...
      <CardContent className="pt-6">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="flex items-center gap-3 p-3 rounded-lg bg-gradient-to-r from-emerald-50 to-transparent hover:from-emerald-100 transition-colors border border-emerald-200">
            ...
            <div className="flex flex-col">
              <span className="text-xs text-emerald-600 font-medium">تاريخ الإفراج</span>
              <span className="text-sm font-bold text-emerald-700">{prisoner.releaseDate || "غير محدد"}</span>
            </div>
          </div>

          <div className="flex items-center gap-3 p-3 rounded-lg bg-gradient-to-r from-teal-50 to-transparent hover:from-teal-100 transition-colors border border-teal-200">
            ...
            <div className="flex flex-col">
              <span className="text-xs text-teal-600 font-medium">الرقم القومي</span>
              <span className="text-sm font-bold text-teal-700 font-mono">{prisoner.nationalId || "غير محدد"}</span>
            </div>
          </div>
        </div>
        ...
        <Dialog open={isOpen} onOpenChange={setIsOpen}>
          <DialogTrigger asChild>
            <Button
              className="w-full mt-4 bg-gradient-to-r from-emerald-500 to-green-600 hover:from-emerald-600 hover:to-green-700 text-white shadow-md transition-all duration-300 hover:shadow-lg"
              size="lg"
            >
          ...
          <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
            <DialogHeader>
              <DialogTitle className="text-2xl font-bold text-right bg-gradient-to-l from-emerald-600 to-green-800 bg-clip-text text-transparent">
                البيانات الكاملة - {prisoner.name}
              </DialogTitle>
              <DialogDescription className="text-right">جميع المعلومات التفصيلية للمفرج عنه</DialogDescription>
            </DialogHeader>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="p-4 rounded-lg bg-gradient-to-r from-emerald-50 to-emerald-100/50 border border-emerald-200">
                  <div className="flex items-center gap-3 mb-2">
                    ...
                    <span className="text-sm font-semibold text-emerald-900">الاسم الكامل</span>
                  </div>
                  <p className="text-lg font-bold text-emerald-700 mr-13">{prisoner.name}</p>
                </div>

                <div className="p-4 rounded-lg bg-gradient-to-r from-green-50 to-green-100/50 border border-green-200">
                  <div className="flex items-center gap-3 mb-2">
                    <div className="w-10 h-10 rounded-full bg-gradient-to-br from-green-500 to-green-600 flex items-center justify-center">
                      <MapPin className="h-5 w-5 text-white" />
                    </div>
                    <span className="text-sm font-semibold text-green-900">السجن</span>
                  </div>
                  <p className="text-lg font-bold text-green-700 mr-13">{prisoner.prison}</p>
                </div>

                <div className="p-4 rounded-lg bg-gradient-to-r from-teal-50 to-teal-100/50 border border-teal-200">
                  <div className="flex items-center gap-3 mb-2">
                    <div className="w-10 h-10 rounded-full bg-gradient-to-br from-teal-500 to-teal-600 flex items-center justify-center">
                      <Users className="h-5 w-5 text-white" />
                    </div>
                    <span className="text-sm font-semibold text-teal-900">العائلة</span>
                  </div>
                  <p className="text-lg font-bold text-teal-700 mr-13">{prisoner.family || "غير محدد"}</p>
                </div>
                ...
                <div className="p-4 rounded-lg bg-gradient-to-r from-cyan-50 to-cyan-100/50 border border-cyan-200">
                  <div className="flex items-center gap-3 mb-2">
                    ...
                    <span className="text-sm font-semibold text-cyan-900">تاريخ الإفراج</span>
                  </div>
                  <p className="text-lg font-bold text-cyan-700 mr-13">{prisoner.releaseDate || "غير محدد"}</p>
                </div>
